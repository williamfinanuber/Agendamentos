...
export const getAvailabilityForProcedure = async (procedureId: string): Promise<Availability> => {
    const [settings, confirmedBookings, allProcedures, blockedSlots] = await Promise.all([
        getAvailabilitySettings(),
        (await getBookings()).filter(b => b.status === 'confirmed' || b.status === 'completed'),
        getProcedures(),
        getBlockedSlots()
    ...
    const targetProcedure = allProcedures.find(p => p.id === procedureId);
    if (!targetProcedure) {
        return {};
    }
    const procedureDuration = targetProcedure.duration;
    const requiredSlots = Math.ceil(procedureDuration / settings.slotInterval);
    ...
    const blockedSlotsMap = new Map(blockedSlots.map(bs => [bs.id, new Set(bs.times)]));
    ...
    for (let i = 0; i < 60; i++) {
        ...
        if (activeWeekdays.includes(dayOfWeek)) {
            const dateKey = date.toISOString().split('T')[0];
            const allDaySlots = generateTimeSlots(settings.startTime, settings.endTime, settings.slotInterval);
            
            // Create a set of all unavailable slots for quick lookup
            const unavailableSlots = new Set<string>();

            // Add manually blocked slots
            const manuallyBlocked = blockedSlotsMap.get(dateKey);
            if (manuallyBlocked) {
                manuallyBlocked.forEach(s => unavailableSlots.add(s));
            }

            // Add slots from existing bookings
            const bookingsForDate = confirmedBookings.filter(b => b.date === dateKey);
            bookingsForDate.forEach(booking => {
                const procedure = allProcedures.find(p => p.id === booking.procedureId);
                if (!procedure) return;

                const bookingStart = new Date(`1970-01-01T${booking.time}`);
                const bookingEnd = new Date(bookingStart.getTime() + procedure.duration * 60000);
            ...
            const validStartSlots = allDaySlots.filter((startTime, index) => {
                ...
                if (index + requiredSlots > allDaySlots.length) {
                    return false; // Not enough slots left in the day
                }

                for (let j = 1; j < requiredSlots; j++) {
                    const nextSlot = allDaySlots[index + j];
                    if (unavailableSlots.has(nextSlot)) {
            ...
            if (validStartSlots.length > 0) {
                availability[dateKey] = validStartSlots;
            }
        }
    }
